$(document).ready(function () {
    ...
    console.log("starting unloading")

    // OpenWeather API
    const apiKey = 'c9680636c1e823cd3b530a0750643b18';

    // Selectors for HTML elements to display weather information
    const locations = $('h2#locations');
    const date1 = $('h3#date');
    const weatherPic = $('img#weather-icon');
    const temperature1 = $('span#temperature');
    const humidity1 = $('span#humidity');
    const wind1 = $('span#wind');
    const uvIndex1 = $('span#uv-index');
    const locationsEl = $('div.locationsEl');
   ...
   // Functions to build the URL for the OpenWeather API call
    ...
    function searchWeather(queryURL) {
        ...
        $.ajax({
            ...
            temperature1.html(((response.main.temp - 273.15) * 1.8 + 32).toFixed(1));
            humidity1.text(response.main.humidity);
            wind1.text((response.wind.speed * 2.237).toFixed(1));
            ...
            $.ajax({
                ...
                uvIndex1.text(response.current.uvi);
                uvIndex1.attr('style', `background-color: ${uvColor}; color: ${uvColor === "yellow" ? "black" : "white"}`);
     ...
     function displayLastSearchedCity() {
        if (pastCities[0]) {
            ...
            // if no past searched cities, load San Jose weather data
            let queryURL = buildURLFromInputs("San Jose");
            searchWeather(queryURL);
        }
    }
